...
在三组完整结果产生前，固定一个辅助分组：复用已完成GT query诊断的双端合法位置样本，按两端GT native cell的Chebyshev距离分为0、1…R、超过R，Δ1采用R2，Δ2采用R4。各组统计当前帧自动定位的双方都对、救回、新增错误和双方都错；不是重新报告历史GT query检索成绩。缺少双端位置的目标只不进入此辅助分组，完整230目标评价继续保留它们。

该分组由搜索几何定义，不按新模型结果选阈值；不等于球尺寸归一化速度，也不分离相机运动。超过范围的样本很少时直接报告计数，不把它当作充分的大位移验证。实现复用保存的GT诊断JSON和逐帧预测CSV，不进行新的图像解码或特征计算。

## 三seed完成后的单次读出宽度控制

2026-09-10追加并在运行前锁定。原hidden32三seed结果保留，不更换其选优指标。新问题是：当前682通道输入共同压到32通道，是否限制已有cost在细定位中的使用？仅将现有`hidden_channels`设为64，固定seed0，对stack、centered、self_centered三组各运行30epoch；其余数据、缓存、目标、归一化、输出网格、优化器、lr、batch16和选优不变。无需新模块或新缓存，不增加raw组，也不做宽度网格搜索。

主要比较在新宽度内部进行：centered64必须PCK8不低于stack64及self64，同时PCK16严格高于两者，才进入预定seed1/2复核。即使满足，这也只支持读出容量/优化是限制因素之一，不证明信息被hidden32不可逆丢弃。若三组共同提高而跨帧cost没有满足上述条件，或细定位代价持续，则停止本轮局部cost融合扩展，下一阶段转向不含cost的训练/表示基线。不通过事后挑容差或改checkpoint选择挽救本轮结果。

新输出为`outputs/correspondence_probe/width64_{stack,centered,self_centered}_seed0`。同宽度逐帧paired比较及既有hidden32结果一起保留；seed0只能决定是否复核，不能作为最终效果声明。
