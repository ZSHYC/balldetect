...
## 同日补充：时间维度压缩与激活顺序已有明确先例

下一控制涉及时序融合前后的非线性，补读Tran等的《A Closer Look at Spatiotemporal Convolutions for Action Recognition》arXiv v3（2018-04-12，CVPR 2018），重点§3.1–3.5。R(2+1)D将空间与时间卷积分开，在中间加入ReLU，并按中间宽度近似匹配参数量；作者同时讨论了优化与非线性数量的作用。[论文全文](https://arxiv.org/pdf/1711.11248v3) 因此，相同参数数量不能自动意味着相同函数类或优化难度，换融合顺序也不是新的研究概念。

本文的R2D将输入帧折叠到通道，f-R2D逐帧共享处理后再池化；这与本项目要区分的时间维保留有关，但动作分类上的结论不能直接移作球中点精度证据。本项目各时间槽有独立投影列块，即使求和后不再有显式时间轴，后续仍可能解码已编码的时序关系，不能笼统写成“压缩后完全没有motion”。这是本地计算图推论，不是对作者实验的重新解释。

本次CVF PDF返回403，改读可访问的arXiv v3全文；未将失败访问计作全文阅读。同期面向近期arXiv的融合/微小目标查询没有找到足以替代上述最近计算图对照的直接来源，这不证明新颖性；既有2025–2026微小运动工作仍按前文的专题记录作为约束。
