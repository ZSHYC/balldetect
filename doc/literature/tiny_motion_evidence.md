...
同日BlurBall配对出现大位移/长拖影获益、小位移退步后，定向补查了DQAligner/MIST的IEEE、作者公开入口、机构作者稿与arXiv，仍未取得可读正文。以下固定作者实现的机制边界继续有效；具体模块消融、错误匹配拒绝与可比预算仍不能据摘要补写。不再重复同一路径的无效获取，新的公开来源或实际采用相邻结构时再更新；当前可继续用本地证据设计诊断，不由这项访问限制推断已有方法不足。
...
### 1.8 MCATrack — 微小目标的递归变化增强与模板更新

**2026-09-12新增，A：ICCV 2025官方全文。** 它是首帧框已知的红外单目标tracker，将递归高通响应与动态模板结合；不是无需初始化的逐帧球发现。公式、实际四组合消融、配准依赖和作者实现缺口见[差分与递归高通专题](2026-09-12-temporal-difference-representation.md#mcatrack有状态的递归高通本身仍然线性)。

该先例说明便宜的时间变化增强已用于微小目标杂波场景。其固定高通公式本身线性；与当前晚融合三帧特征的差异还涉及历史范围、图像级插入位置及配准，不能仅用“递归”宣称非线性，也不能用本地有限条件的换基等价把它直接排除。模板更新的20 px门限不等于定位搜索半径。

### 1.9 CHAL — 背景异常、时序验证与残差门控

**2026-09-12新增，A：CVPR 2026正式主文、补充与固定作者源码。** [完整审查](2026-09-12-causal-anomaly-gating.md)记录了背景特征场、异常图、默认过去五帧预测末帧、检测监督及有效消融。该自动稠密检测器不需要GT初始化；公开代码的低异常分支是残差门控，并非硬删候选。

它使“历史用于背景正常性判别”成为显式对应之外的实质竞争解释。但门控收益不等于因果识别：补充从异常代理收敛推出权重趋于1的关键步骤没有成立，实际重加权也尚未证明等价于后门积分。不能因为缺少物理干预实验就否定标准观察数据因果推断；应审查具体变量、条件与估计量。

论文与公开默认代码在可学习系数、时间坐标、shifted窗口、训练/推理读出和回归损失上有差别。专题按来源分别报告，没有运行CHAL或据此改变当前BlurBall对照，也不由源码差异否定其已报告的检测正结果。
