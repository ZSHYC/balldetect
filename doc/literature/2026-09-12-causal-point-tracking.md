...
## 同日补检 Point4D：保留查询不等于保留当前视觉支持

Minsik Jeon 等的 [*Point4D: Long-range 4D Motion Reconstruction*](https://arxiv.org/abs/2609.09145v1)于2026-09-08首发。本次读方法§3、主要对照与Table3、§5限制；PDF/文本留在 `outputs/literature/point4d-2609.09145v1.*`，未审计代码或运行模型。

它把查询写为3D位置、时间/相机索引和外观patch，以当前块的视觉表示预测轨迹。Algorithm1从第0帧已给定的可见点初始化；跨块只传预测3D坐标和最初patch，不持续提取新外观。实验用48帧块、8帧重叠，块间由预测深度估计Sim(3)对齐，不能省略为“只传一个坐标即可免费长时跟踪”。[§3.2–3.3](https://arxiv.org/html/2609.09145v1#S3)

Table3的Dynamic Replica长视频采用尺度对齐后的3D EPE，2D query为0.712m，仅改3D/source-patch为0.825m，完整版本为0.155m。完整版本还用任意可见帧的patch训练，从而让source时刻可以遮挡/出界；增益不能只归于坐标从2D变3D，训练查询分布也变了。[Table3](https://arxiv.org/html/2609.09145v1#S4.SS3)

作者明确承认：目标整块不可见时，当前块缺乏支持，预测会不可靠。可传递的3D查询让推理过程继续，并未保证它有视觉依据。这为“身份状态、可见支持和位置正确性分开”提供新先例；当前三帧无状态球模型没有该跨块问题，不因此增加3D重建或长记忆。[§5](https://arxiv.org/html/2609.09145v1#S5)
