...
当前输出可视为 \(K=288\cdot512\) 个位置类别加一个 missing 类。空间 argmax 是该分类目标的 mode；把 logits 重新归一化并取 expectation，等于在**不同的连续坐标损失/决策规则**下解释同一个分类器。CE会惩罚目标格之外的概率质量，却没有显式要求这些质量按连续距离、Gaussian或拖影几何分布，也没有规定次峰应如何参与中点估计。

这里还要区分训练分布与评价决策。即使假设空间概率已经是真实条件后验，坐标期望最小化的是平方欧氏误差；严格PCK@r的最优决策则最大化预测点r邻域内的后验质量，二者一般不同。原argmax又对应单格分类的决策。这是损失定义的数学区别，不是已证明本模型概率校准良好，也不是批准另加一个邻域质量解码器；它说明不能仅凭“期望更连续”预言PCK或F1必然改善。
