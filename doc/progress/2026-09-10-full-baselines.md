...
native随后从7dcabb4完成30epoch并通过保存结果复核，选epoch24，PCK@8为82.6460%、F1@16为86.3649%，相对固定基线提高1.2027与0.1093个百分点。@8救回41、破坏20，五个clip净升、两个下降、两个持平；VC0误报97→102。训练/评价耗时857.23秒、峰值354.02MiB，另计一次固定logits准备成本。pooled已从同版本串行启动，epoch0完整复现基线；等待对照完成才判断高空间带宽是否值得保留。
...
最后pooled seed2完成并通过核对，选epoch0，六次运行全部结束。native/pooled的PCK@8描述均值为82.1497%/81.6915%，F1@16为86.2965%/86.3614%；两seed个体方向通过，但平均F1失败，结束该固定残差配方。保留有限位置增量及其边界，下一项独立问题转向共同目标上的历史年龄、可见性与固定搜索支持域，只读元数据，不直接增加motion结构。

[历史年龄诊断](../experiments/2026-09-11-history-age.md)随后从e735f41完成，仅读元数据和写出窗口表耗时1.060秒。固定共同目标上，s=2训练VC1的1,067/10,212例仍有可见历史但旧R2/R4均不覆盖，分布于52/56 clips；s=1仅95例，无VC1历史比例则近乎不变。验证s=2对应317/1,581例。结果区分了范围压力与历史可见性消失，却没有给出弃用近帧的理由；不据此启动长间隔训练或扩搜索模型。无GPU、图像解码或最终测试标签读取，合成检查、独立实现审查和实际窗口／分母断言通过。
