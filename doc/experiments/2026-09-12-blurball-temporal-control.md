...
日期：2026-09-12。状态：完成21轮后意外中断；用户取消重训，转为探索性配对。
...
原进程完成epoch0–21的验证，在epoch22处理3,200个训练目标后因退出中断。完成轮次累计23,476.06秒，约6.52小时，不含未完成epoch22。按原argmax F1@4、其次F1@8、最早同分规则，已完成轮次的最佳为epoch1：PCK@4为71.9448%，本地F1@4为69.5839%。这些是原日志记录，保存权重的重新导出与完整配对结果见后续增量。
