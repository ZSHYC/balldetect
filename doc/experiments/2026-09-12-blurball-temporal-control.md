...
设计审查考虑过“移动两历史帧的GT球迹邻域，并配背景像素交换对照”，本轮决定不实施。它会同时搬运邻域背景、手拍等内容，预测沿移动方向响应也可能来自历史位置外推；这些结果到具体结构选择的映射仍不够明确。保留像素集合并不保留卷积或GroupNorm后的特征分布。这里没有运行该干预，也不将其作为负实验。下一项设计转向当前读出已有源码证据所指的具体函数差异：在有限通道预算下，跨地址非线性交互是否比同址交互后线性空间汇聚更有效，并同时保留容量与计算量的竞争解释。该设计尚未实现，不因这一假设宣布新的motion机制成立。
