...
日期：2026-09-11。状态：六次30epoch及保存结果核对全部完成；三seed复核未通过预定联合条件，停止该固定残差配方的扩展。
...
pooled seed2已完成30epoch并通过保存结果核对，最终选epoch0，完整验证指标等于固定基线；耗时1,233.0662秒，峰值354.02MiB。[同seed配对](../../outputs/full_heatmap/dino_frozen_detail_pooled_vs_native_seed2.json)因此与native seed2对原基线的配对相同。最后一臂没有改变已经确定的均值判定。

| 读出seed | native选优epoch | native PCK@8 | native F1@16 | pooled选优epoch | pooled PCK@8 | pooled F1@16 |
|---|---:|---:|---:|---:|---:|---:|
| 0（探索） | 24 | 82.6460% | 86.3649% | 17 | 82.0160% | 86.3406% |
| 1（复核） | 0 | 81.4433% | 86.2557% | 25 | 81.6151% | 86.4880% |
| 2（复核） | 27 | 82.3597% | 86.2690% | 0 | 81.4433% | 86.2557% |
| 选定run描述均值 | — | 82.1497% | 86.2965% | — | 81.6915% | 86.3614% |

逐seed原四项方向为通过/未通过/通过，但平均F1没有超过pooled，故完整复核条件失败。native平均PCK@8高0.4582个百分点、F1@16低0.06492个百分点；PCK@16均值为88.3543%对88.1634%，PCK@32为90.2635%对89.9580%。这些位置方向仍保留，不能为了停止方案就把全部结果写成无收益。

[汇总JSON](../../outputs/full_heatmap/frozen_detail_readout_seeds.json)保存六次选优、完整位置/检测/存在指标、各同seed配对、描述均值与判定。三组pooled→native的@8净变化为+11、−3、+16个同game目标；这是优化扰动下重复评价同一批帧，不是三倍独立样本，也不是三个不同backbone训练。

本轮到此结束。已观察到有限严格位置读出增量，但没有得到预定的联合检测收益支持，故不把这个残差并入新motion系统，不追加读出seed或超参。本结论只约束固定原表示、这个残差与当前开发协议；不证明浅层信息不存在，也不替代原有真实历史约9pp增量的机制解释。
