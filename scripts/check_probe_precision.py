...
def main():
    ...
    parser.add_argument("--runs", type=Path, nargs=4, required=True, help="按 stage 0..3 排列的已完成运行")
    ...
    models = []
    for stage, run in enumerate(args.runs):
    ...
    with torch.inference_mode():
        for pixels in loader:
            ...
            for stage, (model, feature) in enumerate(zip(models, features)):
                for precision, x in enumerate((feature, feature.half().float())):
                    logits = model(x)
                    positions[stage][precision].extend(logits[:, :-1].argmax(1).cpu().tolist())
                    probabilities[stage][precision].extend((1 - logits.softmax(1)[:, -1]).cpu().tolist())
    results = {"frames": len(rows), "scope": "game7 sampled validation; fixed trained linear heads; float32 forward vs float16 storage",
               "runs": [str(p) for p in args.runs], "stages": {}}
    for stage in range(4):
        p32, p16 = np.array(positions[stage])
        q32, q16 = np.array(probabilities[stage])
        grid_hw = tuple(s * models[stage].upscale for s in meta["shapes"][stage][-2:])
