"""用cross-address固定best在全部BlurBall训练目标上一次性导出K16候选。"""
import argparse
import json
...
from compare_blurball_temporal import _validate_saved_rows, continuous_windows, read_predictions
...
def main():
    ...
    if config['temporal_input'] != 'history':
        raise ValueError('--run必须使用真实历史输入')

    cache = ROOT / config['rgb_cache']
    metadata = json.loads((cache / 'metadata.json').read_text())
    frames = metadata['frames']
    all_windows, _ = continuous_windows(
        frames, metadata['windows'], config['continuity_boundaries'])
    windows = np.asarray(
        [window for window in all_windows if frames[window[-1]]['split'] == 'train'],
        dtype=np.int64)
    rows = [frames[window[-1]] for window in windows]
    if len(rows) != int(config['train_targets']) or len(rows) != 38854:
        raise ValueError('训练目标数与cross运行协议不一致')

    prediction_path = args.run / 'train_predictions.csv'
    ...
    model = build_dino_model(
        config['weights'], upscale=8, interaction='cross_address').to(device)
    ...
    with torch.inference_mode():
        arrays = extract_candidate_arrays(
            model, rgb, windows, rows, device, config['batch_size'], progress_label='train')
    forward_seconds = time.perf_counter() - forward_started

    if not np.array_equal(arrays['original_xy'][:, 0], original_xy):
        raise ValueError('K1 argmax没有复现全部train_predictions')
    ...
    if not np.array_equal(arrays['q'] >= .5, original_q >= .5):
        raise ValueError('q阈值输出没有复现全部train_predictions')
    ...
    manifest = {
        ...
        'code_changes': ['scripts/probe_blurball_candidates.py: shared extraction',
                         'scripts/export_blurball_candidates.py: train export'],
        'source_run': str(args.run.resolve()),
        'source_checkpoint': str((args.run / 'best.pt').resolve()),
        'checkpoint_epoch': checkpoint['epoch'],
        'interaction': 'cross_address',
        'input': ['t-2', 't-1', 't'],
        'input_frame_offsets': [-2, -1, 0],
        'temporal_scope': 'causal continuous windows within one clip/rally; target is t',
        'target_split': 'train',
        ...
        'local_reference_available': False,
